...
- Baseline release: `0.7.33-beta` (`versionCode=1058`)
- Last verified device: Fire TV `AFTMM` (`mantis`)
- Product direction: TMDB owns discovery and metadata; authorized Xtream content owns playback availability
- Implementation status: Phase 1, Phase 2, and Phase 3 remain TMDB-first with Xtream playback filtering; `0.7.32-beta` resolves Cinemeta Discover previews through TMDB before rendering, so identity, configured-language title, localized artwork, metadata, and Xtream availability all use the same localized TMDB candidate. Exact playback-cache updates also trigger visible badge reclassification. Home and its See All remain filtered and badge-free. `0.7.33-beta` adds Xtream live TV channels and parental control (see [`.specs/STATE.md`](.specs/STATE.md) AD-006); the Home/catalog phases above are unaffected. Fire TV visual/playback validation remains user-owned and pending.
...
- TMDB is the source for discovery, search, metadata, artwork, seasons, and episodes.
- Xtream is indexed locally and used to resolve movie and episode playback.
...
- In-app OTA updates are enabled for the full flavor: release builds check the `jeziel-jr/Lume` GitHub Releases channel on every launch (silent on failure), and the manual check remains available in About; every push to main publishes a signed beta release automatically.
- The Xtream provider endpoint is operator-controlled: `remote-config/xtream.json` plus a detached ECDSA P-256 signature is fetched from the public repository, verified against an embedded key, and applied on startup, on a six-hour TTL, on connectivity failure, or on demand from Settings. Users no longer type or edit a server; a hidden five-OK gesture reveals a manual override for the operator alone, marked and resettable in Settings.
...
Implementation: complete in `0.7.20-beta`. A compressed last-known-good snapshot is indexed off the UI thread, keyed by schema and a credential-free endpoint/user fingerprint, reused while stale, and refreshed without replacing valid data on failure or implausibly small responses. Fire TV size/load-time measurements remain to be recorded. Operator endpoint rotation reuses this machinery: `XtreamCatalogRepository.switchEndpoint()` keeps the previous snapshot and index serving while the new endpoint is downloaded and re-indexed, and replaces them atomically on success.
...
Implementation complete through `0.7.32-beta`: exact movie and series/episode availability results persist locally, focused-card prefetch remains debounced, Home filters confirmed or conservatively unmatched titles through the in-memory Xtream index, and sparse rails expand to a maximum of five TMDB pages. Title surfaces outside Home keep the full result set and share the approved charcoal, ivory, and amber `Indisponível`/`Verificando` badge across Search and See All, Discover, filmography, network/company browse, related titles, TMDB collections, Library, and local folders/collections. Home and Home-origin See All remain filtered and badge-free. The reusable tracker follows index readiness and exact playback-cache revisions, reclassifies pagination, cancels stale results, and emits no false unavailable state on index failure. Cinemeta IMDb previews in Discover are resolved through TMDB before rendering, replacing English addon title/artwork with configured-language TMDB title, poster, backdrop, and metadata while retaining original titles for conservative matching. IMDb movies and `tt…:season:episode` identifiers resolve to TMDB before Xtream playback; non-convertible legacy IDs retain their fallback. Movie playback accepts a strict title/year candidate when the provider detail has a missing or zero TMDB ID, but still rejects any positive ID that points to a different TMDB movie.
...
8. Keep all TMDB titles discoverable outside Home, show `Indisponível` before navigation, and allow details to open with playback disabled. Home and Home-origin See All remain filtered and do not show badges.
...
- Every supported non-Home title surface visually distinguishes unavailable and checking states without marking playable titles; Home remains badge-free.
...
- Hide unavailable titles on Home; keep them discoverable on non-Home title surfaces with the approved badge and accessible metadata details.
